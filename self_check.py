...
def main() -> None:
    ...
    checks = [
        ...
        ("Desktop GUI present", Path("gui_app.py").exists() and Path("gui_data.py").exists() and "customtkinter" in Path("bootstrap.py").read_text(encoding="utf-8") and "filter_schedule_rows" in Path("gui_app.py").read_text(encoding="utf-8")),
        ("GUI exposes all schedule ranges", "NUMERIC_FILTERS" in Path("gui_app.py").read_text(encoding="utf-8") and "average" in Path("gui_data.py").read_text(encoding="utf-8") and "reports" in Path("gui_data.py").read_text(encoding="utf-8")),
        ...
        ("All ranked guilds scanned through own rank", '"scan_all_ranked_guilds_to_own": true' in config_text and "raiderio_{ranking_scope}_ranked_to_own" in Path("schedule_scan.py").read_text(encoding="utf-8") and "int(item.rank) <= int(cutoff_rank)" in Path("schedule_scan.py").read_text(encoding="utf-8")),
